...
// Function to count the nodes in the tree (used to maintain binary tree structure)
int countNodes(Node* root) {
    if (root == NULL) return 0;
    return 1 + countNodes(root->left) + countNodes(root->right);
}

// Function to insert a node into a Binary Tree (maintaining level-order)
Node* insert(Node* root, int data) {
    ...
    // Insert into the left or right subtree, whichever has fewer nodes
